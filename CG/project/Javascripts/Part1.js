...
window.onload = function init()
{
    ...
    document.getElementById("reflectSelect").onchange = function() {
        var element = document.getElementById("reflectSelect");
        data.interreflections = parseInt(element.options[element.selectedIndex].value);
    }
...
function generateCubeMaps() {
    // clear all cube maps initially
    for (var i = 0; i < data.mirrorBalls.length; i++) {
        resetCubemap(data.mirrorBalls[i]);
    }

    for (var j = 0; j < data.interreflections; j++) {
        // first we render all cube maps
        for (var i = 0; i < data.mirrorBalls.length; i++) {
...
function renderScene(V, P, ignoreId) {
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

    renderCornellBox(V, P);

    // render the mirror balls
    switchProgram("MirrorBall");
    for (var i = 0; i < data.numOfBalls; i++) {
        if (ignoreId != i)
            renderMirrorBall(data.mirrorBalls[i], V, P);
    }

}

function renderCornellBox(V, P) {
    ...
    // don't store depth (used in parallax mapping)
    gl.uniform1f(lambertianProgram.storeDepthLocation, 0);
...
function renderMirrorBall(mirrorBall, V, P) {
    // set matrices
    ...
    gl.uniform1f(mirrorBallProgram.storeDepthLocation, 0);
    gl.uniform1f(mirrorBallProgram.useParallaxLocation, 0);
