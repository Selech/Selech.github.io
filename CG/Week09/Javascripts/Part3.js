...
function applyAnimation() {
    ...
    if (data.bouncing) {
        if (data.translateY > 1.0 || data.translateY < -0.8)
...
function renderReflection(VLight, lightPos) {
    ...
    // change the near-plane to the reflector
    var P = clipProjection();
    gl.uniformMatrix4fv(phongProgram.PLocation, false, flatten(P));
...
function clipProjection() {
    ...
    for (var i = 0; i < 4; i++) {
        Ndots[i] = dot(data.V[i], N);
        p0dots[i] = dot(data.V[i], p0);
    }

    var d = -dot(p0dots, Ndots);
    var clipplane = vec4(Ndots[0],Ndots[1],Ndots[2], d);
    return modifyProjectionMatrix(clipplane, data.P);
}
